...
const savePlan = async (req, res) => {
  try {
    ...
    const dbData = {
      ...
      task_days: Array.isArray(planData.task_days)
        ? planData.task_days.join(",")
        : planData.task_days || "",
      messages: Array.isArray(planData.messages)
        ? planData.messages.join("; ")
        : planData.messages || "",
    };
...
const updateMessages = async (req, res) => {
  try {
    ...
    if (!plan_id || !messages || typeof messages !== "string") {
      return res
        .status(400)
        .json({ success: false, message: "Invalid plan_id or messages" });
    }
  ...
  try {
    ...
    if (!plan_id || !message || typeof message !== "string") {
      return res
        ...
        .json({
          success: false,
          message: "Invalid plan_id or approval message",
        });
    }
