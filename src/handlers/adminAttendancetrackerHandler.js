...
const getMissingPunchInEmployeesHandler = async (req, res) => {
  try {
    const missingPunchInEmployees =
      await adminAttendencetrackerService.getMissingPunchInEmployees();
    ...
    res
      ...
      .json({
        success: false,
        message: "Error fetching missing punch-in employees",
      });
  }
};
...
const getEmployeesWithPunchInNotPunchedOutHandler = async (req, res) => {
  try {
    const employeesWithPunchInNotPunchedOut =
      await adminAttendencetrackerService.getEmployeesWithPunchInNotPunchedOut();
    res
      .status(200)
      .json({ success: true, data: employeesWithPunchInNotPunchedOut });
  } catch (error) {
    console.error(
      "Error in getEmployeesWithPunchInNotPunchedOutHandler:",
      error
    );
    res
      .status(500)
      .json({
        success: false,
        message: "Error fetching employees with punch-in but not punched out",
      });
  }
};
const getEmployeesWorkedLessThan8HoursHandler = async (req, res) => {
  try {
    const employees =
      await adminAttendencetrackerService.getEmployeesWorkedLessThan8Hours();
    ...
    res
      ...
      .json({
        success: false,
        message: "Error fetching employees who worked less than 8 hours",
      });
  }
};
...
const getEmployeesWorked8To10HoursHandler = async (req, res) => {
  try {
    const employees =
      await adminAttendencetrackerService.getEmployeesWorked8To10Hours();
    res.status(200).json({ success: true, data: employees });
  } catch (error) {
    ...
    res
      ...
      .json({
        success: false,
        message: "Error fetching employees who worked 8 to 10 hours",
      });
  }
};
...
const getApprovedLeavesCurrentMonthHandler = async (req, res) => {
  try {
    const approvedLeaves =
      await adminAttendencetrackerService.getApprovedLeavesCurrentMonth();
    ...
    res
      ...
      .json({
        success: false,
        message: "Error fetching approved leaves for the current month",
      });
  }
};
