...
/**
 * GET /old/download/:claimId
 * Generates DOCX and converts to PDF, returns as download
 */
exports.generateReimbursementPDF = async (req, res) => {
  try {
    ...
    // Filter attachments that have file_path
    const attachmentsWithFiles = attachments.filter(
      (a) => a.file_path && fs.existsSync(a.file_path)
    );
    ...
    const fileName = `${(employee.name || "reimbursement").replace(
      /\s+/g,
      "_"
    )}.pdf`;
    res.download(pdfPath, fileName, (err) => {
      if (err) console.error("generateReimbursementPDF download error:", err);
      // cleanup temp files if generated by services
      try {
    ...
    console.error("generateReimbursementPDF error:", err);
...
/**
 * PUT /old/reimbursement/payment-status/:id
 */
...
/**
 * GET /old/reimbursements
 * Admin: returns grouped reimbursements
 */
...
/**
 * GET /old/team/:teamLeadId?departmentId=...&submittedFrom=...&submittedTo=...
 */
