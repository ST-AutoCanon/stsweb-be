...
async function sendAttendanceReport(startDate, endDate, label = "") {
 ...
 if (!API_KEY) throw new Error("X_API_KEY is not defined in the .env file");
 if (!EMPLOYEE_ID) throw new Error("EMPLOYEE_ID is not defined in the .env file");
 ...
 const requests = [
 axios.get(`${process.env.REACT_APP_BACKEND_URL}/admin/attendance/missing-punch-in?start=${startDate}&end=${endDate}`, { headers })
 .catch(err => { 
 console.error("Missing Punch-in Error:", err.message, err.response?.data?.error || err.response?.data); 
 throw err; 
 }),
 axios.get(`${process.env.REACT_APP_BACKEND_URL}/admin/attendance/punch-in-not-punched-out?start=${startDate}&end=${endDate}`, { headers })
 .catch(err => { 
 console.error("Punch-in Only Error:", err.message, err.response?.data?.error || err.response?.data); 
 throw err; 
 }),
 axios.get(`${process.env.REACT_APP_BACKEND_URL}/admin/attendance/worked-less-than-8-hours?start=${startDate}&end=${endDate}`, { headers })
 .catch(err => { 
 console.error("Worked < 8 Hours Error:", err.message, err.response?.data?.error || err.response?.data); 
 throw err; 
 }),
 axios.get(`${process.env.REACT_APP_BACKEND_URL}/admin/attendance/worked-8-to-10-hours?start=${startDate}&end=${endDate}`, { headers })
 .catch(err => { 
 console.error("Worked 8-10 Hours Error:", err.message, err.response?.data?.error || err.response?.data); 
 throw err; 
 }),
 axios.get(`${process.env.REACT_APP_BACKEND_URL}/admin-attendance/approved-leaves-current-month`, { headers })
 .catch(err => { 
 console.error("Approved Leaves Error:", err.message, err.response?.data?.error || err.response?.data); 
 throw err; 
 }),
 ];

 const [missingPunchInRes, punchInOnlyRes, workedLessRes, worked8to10Res, approvedLeavesRes] = await Promise.all(requests);
 ...
 let startRowSheet1 = null;
 rows.forEach((row, index) => {
 const rowNumber = index + 2;
 if (currentDate === row.attendance_date.toISOString().slice(0, 10)) {
 // Continue merging
 } else {
 if (startRowSheet1 !== null && startRowSheet1 !== rowNumber - 1) {
 sheet1.mergeCells(`C${startRowSheet1}:C${rowNumber - 1}`);
 }
 currentDate = row.attendance_date.toISOString().slice(0, 10);
 startRowSheet1 = rowNumber;
 }
 });

 if (startRowSheet1 !== null && startRowSheet1 !== rows.length + 1) {
 sheet1.mergeCells(`C${startRowSheet1}:C${rows.length + 1}`);
 }

 // Sheet 2: Punch-in without Punch-out
 const sheet2 = workbook.addWorksheet("Punch-in without Punch-out", {
 properties: { tabColor: { argb: "FFFFC000" } }, // Orange
 });
 ...
 let lastDateSheet3 = null;
 let mergeStartRowSheet3 = null;
 ...
 if (currentDate === lastDateSheet3) {
 // Same as previous
 } else {
 if (mergeStartRowSheet3 !== null && mergeStartRowSheet3 !== rowNum - 1) {
 sheet3.mergeCells(`C${mergeStartRowSheet3}:C${rowNum - 1}`);
 }
 mergeStartRowSheet3 = rowNum;
 lastDateSheet3 = currentDate;
 }
 }

 if (mergeStartRowSheet3 !== null && mergeStartRowSheet3 !== sheet3.rowCount) {
 sheet3.mergeCells(`C${mergeStartRowSheet3}:C${sheet3.rowCount}`);
 }
 ...
 let lastDateSheet4 = null;
 let mergeStartRowSheet4 = null;
 ...
 .sort((a, b) => new Date(a.attendance_date) - new Date(b.attendance_date))
 .forEach((emp, index) => {
 const currentDate = new Date(emp.attendance_date).toISOString().split("T")[0];
 ...
 sheet4.mergeCells(`C${mergeStartRowSheet4 + 2}:C${index + 2}`);
 }
 mergeStartRowSheet4 = index;
 lastDateSheet4 = currentDate;
 }
 });

 if (mergeStartRowSheet4 !== null && mergeStartRowSheet4 !== worked8to10.length - 1) {
 sheet4.mergeCells(`C${mergeStartRowSheet4 + 2}:C${worked8to10.length + 1}`);
 }
 ...
 let currentRowSheet5 = 2;
 let previousDateSheet5 = null;
 let mergeStartRowSheet5 = null;
 ...
 if (formattedLeaveDate === previousDateSheet5) {
 // Continue merge block
 } else {
 if (previousDateSheet5 && currentRowSheet5 - mergeStartRowSheet5 > 1) {
 sheet5.mergeCells(`F${mergeStartRowSheet5}:F${currentRowSheet5 - 1}`);
 }
 mergeStartRowSheet5 = currentRowSheet5;
 previousDateSheet5 = formattedLeaveDate;
 }

 currentRowSheet5++;
 });

 if (filteredLeaves.length && currentRowSheet5 - mergeStartRowSheet5 > 1) {
 sheet5.mergeCells(`F${mergeStartRowSheet5}:F${currentRowSheet5 - 1}`);
 }
 ...
 console.error("❌ Error sending attendance report:", error.message, error.response?.data?.error || error.response?.data);
 }
}
...
// For testing: schedule "1st to 14th" job after 1 minute
const now = new Date();
...
cron.schedule(`${testMinute} ${testHour} * * *`, () => {
 console.log("🧪 Running TEST: 1st to 14th report...");
 const today = new Date();
 const year = today.getFullYear();
 const month = today.getMonth();
 const start = new Date(year, month, 1).toISOString().slice(0, 10); // 2025-06-01
 const end = new Date(year, month, 14).toISOString().slice(0, 10); // 2025-06-14
