...
const getEmployeeProjects = async (employeeId) => {
    ...
    const [rows] = await db.query(queries.GET_EMPLOYEE_PROJECTS, [jsonEmployeeId]);
    return rows;
  };  
...
const fetchEmployees = async () => {
    try {
      const [employees] = await db.execute(queries.GET_EMPLOYEES_WITH_DEPARTMENT);
      return employees;
    } catch (error) {
      throw error;
    }
