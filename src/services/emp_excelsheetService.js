...
} = require("../constants/emp_excelsheetQueries");
...
const getAttendanceExcelData = async (fromDate, toDate) => {
  ...
  worksheet.columns = [
    ...
    {
      header: "First Punch In Location",
      key: "first_punchin_location",
      width: 25,
    },
    {
      header: "Last Punch Out Location",
      key: "last_punchout_location",
      width: 25,
    },
  ];
  ...
  rows.forEach((row) => {
    worksheet.addRow({
      ...
      first_punchin: row.first_punchin
        ? new Date(row.first_punchin).toLocaleString()
        : "—",
      last_punchout: row.last_punchout
        ? new Date(row.last_punchout).toLocaleString()
        : "—",
