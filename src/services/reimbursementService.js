...
exports.getReimbursementsByEmployee = async (employeeId, fromDate = null, toDate = null) => {
    ...
    try {
        ...
        const [attachments] = await db.query(queries.GET_ATTACHMENTS_BY_REIMBURSEMENT_IDS, [reimbursementIds]);

        return mapAttachmentsToReimbursements(reimbursements, attachments, employeeId);
    } catch (error) {
...
const mapAttachmentsToReimbursements = (reimbursements, attachments, employeeId) => {
    const attachmentMap = {};
...
exports.getAllReimbursements = async (fromDate = null, toDate = null) => {
    try {
        const [reimbursements] = await db.query(queries.GET_ALL_REIMBURSEMENTS, [
            fromDate, fromDate, toDate, // Single-date claims
            fromDate, fromDate, toDate, // From date overlaps
            fromDate, fromDate, toDate, // To date overlaps
            fromDate, toDate, fromDate  // Full range inside a claim period
        ]);
...
exports.createReimbursement = async (reimbursementData) => {
    try {
        ...
        if (date) {
            ...
            [existingClaim] = await db.query(queries.CHECK_EXISTING_REIMBURSEMENT_SINGLE_DATE, [
                employeeId, claim_type, date
            ]);
        } else if (from_date && to_date) {
            // **Check if a claim of the same type exists within the date range**
            [existingClaim] = await db.query(queries.CHECK_EXISTING_REIMBURSEMENT_DATE_RANGE, [
                employeeId, claim_type, from_date, to_date, from_date, to_date
            ]);
        }

        if (existingClaim.length > 0) {
            const error = new Error("A reimbursement claim of this type has already been submitted.");
        ...
        const reimbursementArray = [
            ...
            reimbursementData.no_of_days !== undefined ? reimbursementData.no_of_days : 0, // Default to 0
            reimbursementData.transport_amount !== undefined && reimbursementData.transport_amount !== 'undefined' 
            ...
            reimbursementData.da !== undefined && reimbursementData.da !== 'undefined' 
                ...
                : 0, // Convert 'undefined' to 0
            reimbursementData.total_amount !== undefined ? reimbursementData.total_amount : 0,
        ...
        const [result] = await db.query(queries.CREATE_REIMBURSEMENT, reimbursementArray);
...
exports.updateReimbursementStatus = async (id, status, approver_comments, approver_id, approver_name, approver_designation) => {
    ...
    return { id, status, approver_comments, approver_id, approver_name, approver_designation, approved_date: new Date() };
};
...
exports.getTeamReimbursements = async (departmentId, effectiveFromDate, effectiveToDate) => {
    const params = [
        departmentId,
        effectiveFromDate, effectiveFromDate, effectiveToDate,
        effectiveFromDate, effectiveToDate, effectiveFromDate,
    ];

    // Fetch reimbursements
    const [reimbursements] = await db.query(queries.GET_TEAM_REIMBURSEMENTS, params);
